...
4. **如果被 `<task-notification>` 唤醒**而非此提示词：处理事件，然后使用 `${DYNAMIC_MODE_SENTINEL}` 和步骤 3 中相同的 1200–1800s `delaySeconds` 再次调用 ${SCHEDULE_WAKEUP_TOOL_NAME} —— ${MONITOR_TOOL_NAME} 仍然是唤醒信号；这只是重置安全网。
...
6. 简要确认：${CONFIRMATION_MESSAGE}、${MONITOR_TOOL_NAME} 是否为主唤醒信号、以及你选择的后备延迟。
