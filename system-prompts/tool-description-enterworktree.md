...
传递 `path` 而非 `name` 将会话切换到已存在的工作树（例如，你刚刚用 `git worktree add` 创建的）。路径必须出现在当前仓库的 `git worktree list` 中——不属于此仓库注册工作树的路径将被拒绝。通过此方式进入的工作树，ExitWorktree 不会将其删除；使用 `action: "keep"` 返回原始目录。
...
- `path`（可选）：要进入的当前仓库已有工作树的路径，而非创建新工作树。与 `name` 互斥。
