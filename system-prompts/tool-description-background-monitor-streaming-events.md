...
**脚本质量：**
- 每个管道阶段必须逐行刷新，否则匹配项会在其缓冲区中不可见：`grep` 需要 `--line-buffered`，`awk` 需要 `fflush()`。`head` 完全无法刷新——`| head -N` 直到 N 个匹配累积后才会输出任何内容，然后终止流。
...
**输出量**：stdout 的每一行都是一条对话消息，因此过滤器应当有选择性 —— 但选择性意味着"你会据此采取行动的行"，而非"仅好消息"。永远不要直接输出原始日志；只过滤到你关心的成功和失败信号。产生过多事件的监控器会被自动停止；如果发生这种情况，请以更严格的过滤器重新启动。
