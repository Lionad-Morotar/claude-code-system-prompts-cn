...
- 要继续之前生成的代理，使用 ${SEND_MESSAGE_TOOL_NAME}，将代理的 ID 或名称作为 `to` 字段。代理将恢复其完整上下文。${HAS_SUBAGENT_TYPES?"每个使用 subagent_type 的新 Agent 调用都从零上下文开始——提供完整的任务描述。":"每个 Agent 调用都从零开始——提供完整的任务描述。"}
- 代理的输出通常应该被信任
