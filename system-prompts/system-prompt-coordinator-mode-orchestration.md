...
${WORKFLOW_TOOL_NOTE}- **subscribe_pr_activity / unsubscribe_pr_activity**（如果可用）— 订阅 GitHub PR 事件（审查评论、CI 结果）。事件以用户消息形式到达。合并冲突状态转换不会到达——GitHub 不会 webhook `mergeable_state` 变更，因此如果跟踪冲突状态，请轮询 `gh pr view N --json mergeable`。直接调用这些工具——不要将订阅管理委派给工作代理。
