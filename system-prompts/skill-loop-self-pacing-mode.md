...
5. **如果你是被 `<task-notification>` 唤醒的**而非此提示词：在循环任务的上下文中处理事件，然后使用步骤 4 中相同的 `prompt` 和相同的 1200–1800s `delaySeconds` 再次调用 ${SCHEDULE_WAKEUP_TOOL_NAME} —— ${MONITOR_TOOL_NAME} 仍然是唤醒信号；这只是重置安全网。
