...
- 用户在消息中包含了 "ultrawork" 关键词（你会看到系统提醒确认这一点）。
...
对于任何其他任务——即使该任务明显能从并行处理中受益——不要调用此工具。使用 Agent 工具处理单个子代理，或简要描述多代理 workflow 能做什么以及大致成本，并询问用户是否要运行。可以提一下，他们可以在未来的消息中包含 "ultrawork" 来跳过询问。
