...
description: 用于发布计划工件的独立 HTML 模板，包括布局、填充契约和亮/暗色样式
ccVersion: 2.1.212
-->
<!--
name: plan
description: 实施计划、技术设计、RFC 和架构提案 — 包含上下文、方法和验证分阶段的文档。
fill contract: 自动发布路径 (src/frame/planArtifactHtml.ts) 通过固定正则替换 {{TITLE}}、{{TAB_TITLE}}、{{EYEBROW}} 和 {{SUMMARY}}
  （{{TAB_TITLE}} 是 <title> 元素；.md-file 发布保留文件名，而 h1 显示文档标题），
  从第一个 <section> 到最后一个 </section> 的所有内容被渲染的计划主体整体替换。保留这四个槽位和 section 运行，
  在最后一个 </section> 之后除了 </article> 不放置任何内容；test/frame/planArtifactHtml.test.ts 中的测试断言此结构。
style: 令牌来自 @ant/cds 自身的 vanilla 导出，逐字嵌入
  （发布的工件是独立的，因此生成的 tokens.vanilla.css 被内联而非导入；漂移测试保持其规范性）。
  排版刻意使用简单的系统字体栈 — 不使用 Anthropic 品牌字体
  也不使用衬线字体 — 亮色背景为白色而非 CDS 的 cream surface-0；这些是刻意偏离 CDS 值的地方。
  暗色模式基于两个主题轴：prefers-color-scheme 和查看器
  切换的 data-theme 根标记（由下方的脚本镜像到 data-mode，使内嵌令牌块自身的切换选择器为其服务）。
-->
